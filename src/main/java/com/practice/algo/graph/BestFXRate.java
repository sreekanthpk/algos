...
class Edge1 {
    ...
    Edge1(String f, String t, double w) { from = f; to = t; weight = w; }
}

public class BestFXRate {
    ...
    public static Result bestRate(Map<String, List<Edge1>> graph, String src, String dst) {
      ...
      for(int i=0; i<n-1;i++){
        ...
        for (int i = 0; i < graph.size() && curr != null; i++) {
            path.add(curr);
        ...
        double bestRate = Math.exp(-dist.get(dst)); // convert back from -log
        return new BestFXRate.Result(bestRate, path);
